...
# • Terminal  — benchmark × commit matrix with multi-shade RAG colour coding
# • Markdown  — build/jmh/benchmark-results-analysis.md  (arrows + plain numbers)
#

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS_FILE="$SCRIPT_DIR/build/jmh/benchmark-results.csv"
MD_FILE="$SCRIPT_DIR/build/jmh/benchmark-results-analysis.md"

if [ ! -f "$RESULTS_FILE" ]; then
    echo "Error: Results file not found: $RESULTS_FILE"
    echo "Run run-benchmarks.sh first."
    exit 1
fi
